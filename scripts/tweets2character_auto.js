...
program
  ...
  .option('--model <model>', 'Model to use (openai/claude)')
  .option('--user-info <info>', 'User info for the character')
  .parse(process.argv);
...
const resumeOrStartNewSession = async (projectCache, archivePath) => {
  ...
  if (!projectCache.unfinishedSession) {
    ...
    projectCache.model = program.opts().model || 'openai';
    projectCache.basicUserInfo = program.opts().userInfo || 'Matthew Walker, Matt Walker, sleepdiplomat, 51 years, director of the Center for Human Sleep Science  -in UC Berkeley\'s department of psychology';
    ...
    console.log(`Using model: ${projectCache.model}`);
    console.log(`Using user info: ${projectCache.basicUserInfo}`);
  }
...
const main = async () => {
  try {
    ...
    if (!archivePath) {
      console.error('Please provide the path to your Twitter archive zip file as an argument');
      process.exit(1);
    }
    ...
    // Set basicUserInfo for the prompt
    basicUserInfo = projectCache.basicUserInfo;

    // Set API key from command line if provided
    if (program.opts().openai) {
      process.env.OPENAI_API_KEY = program.opts().openai;
    }
    if (program.opts().claude) {
      process.env.CLAUDE_API_KEY = program.opts().claude;
    }
